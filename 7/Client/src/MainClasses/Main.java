...
import Vehicles.*;
import VehiclesThreadTools.OtherThreads.VehicleCreatorRunnable;
import VehiclesThreadTools.OtherThreads.VehicleMarkWriterRunnable;
...
public class Main {
    ...
    private static void main_lab7(String[] args) throws ClassNotFoundException, DuplicateModelNameException,
        ...
        Vehicle vehicleOne = new Motorbike("CatMachine", 2);

        vehicleOne.setModelName(first, vehicleOne.NON_SET_NAME);
        vehicleOne.setPriceByModelName(100.0f, first);

        vehicleOne.setModelName(second, vehicleOne.NON_SET_NAME);
        vehicleOne.setPriceByModelName(10.0f, second);

        Vehicle vehicleTwo = new Car("SecondCatMachine", 1);

        vehicleTwo.setModelName(third, vehicleTwo.NON_SET_NAME);
        vehicleTwo.setPriceByModelName(1.0f, third);

        vehicleTwo.addNewModel(fo, 330.0f);

        Vehicle vehicleThird = new Car("ThirdCatMachine", 0);
        vehicleThird.addNewModel(five, 1.0f);
        vehicleThird.addNewModel(six, 5.0f);


        Vehicle[] vehicles = {vehicleOne, vehicleTwo, vehicleThird};

        int serverPort = 2091;
        String addressHost = "localhost";

        try {
            Socket socket = new Socket(addressHost, serverPort);
            ...
            System.out.println("Received answer from server");

            System.out.println("Avg price: " + in.readFloat());
