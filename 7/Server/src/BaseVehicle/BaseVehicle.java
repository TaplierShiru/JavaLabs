...
public abstract class BaseVehicle implements Vehicle {
    ...
    public void createFromCharsWithScanner(Scanner in) throws DuplicateModelNameException {
        System.out.println("Enter mark: ");
        this.setMark(in.nextLine());
        ...
        String exitTest = in.nextLine();
        while (!exitTest.equals("None")){

            System.out.println("Enter model name");
            String newModelName = in.nextLine();
    ...
    public void writeInChars(Writer out) throws VehicleCreationException {
        ...
        stream.printf("%s\n%d\n", this.getMark(), this.getModelSize());

        String[] arrNames = this.getModelNames();
        float[] arrPrices = this.getModelPrices();

        for (int i = 0;i < arrNames.length; i++){
            stream.printf("New model:\n%s\n%f\n", arrNames[i], arrPrices[i]);
        }
